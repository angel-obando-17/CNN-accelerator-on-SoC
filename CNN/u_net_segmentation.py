...
Este script hace 3 cosas en orden:
  ...
  3. Aplica la U-Net a todas las imágenes del dataset.
...
MASKS_DIR         = "/mnt/c/Users/ANGEL OBANDO/Documents/Trabajo de grado/CNN/mascaras_manual"
DATASET_ROOT      = "/mnt/c/Users/ANGEL OBANDO/Documents/Trabajo de grado/CNN/PlantVillage"
OUTPUT_DIR        = "/mnt/c/Users/ANGEL OBANDO/Documents/Trabajo de grado/CNN/resultados_experimentos/segmentacion"
SEGMENTED_DATASET = "PlantVillage_segmentado"
...
def labelme_json_to_mask( json_path, output_size=None ):
    #Lee un .json de LabelMe y retorna (imagen_bgr, mascara_binaria).
...
def convert_all_masks( ):
    ...
    json_files = sorted( [ f for f in os.listdir( MASKS_DIR ) if f.endswith( ".json" ) ] )
...
def plot_mask_comparisons( pairs, names ):
    #Imagen original vs máscara por cada par.
    n   = len( pairs )
...
def build_unet( input_size=256 ):
    #U-Net ligera para segmentación binaria de hojas. Entrada 256x256, salida binaria.
...
def augment_pair( img, mask ):
    #Data augmentation sobre los 9 pares imagen+máscara.
    augmented = [ ( img, mask ) ]
    ...
    for angle in [ -30, -15, 15, 30 ]:
        ...
        aug_mask = cv2.warpAffine( mask, M, ( w, h ), flags=cv2.INTER_NEAREST, borderMode=cv2.BORDER_REFLECT_101 )
...
def prepare_training_data( pairs ):
    #Arrays X e Y con augmentación. 9 pares → ~81 muestras.
    X, Y = [ ], [ ]
...
def train_unet( pairs ):
    ...
    callbacks = [
        ...
        tf.keras.callbacks.ReduceLROnPlateau( monitor="loss", factor=0.5, patience=7, min_lr=1e-6 )
    ]
...
def evaluate_unet_on_pairs( model, X, Y ):
    #Predicciones vs máscaras manuales sobre las 9 muestras originales.
    n_aug    = len( X ) // 9
    orig_idx = list( range( 0, len( X ), n_aug ) )[ :9 ]

    fig, axes = plt.subplots( len( orig_idx ), 3, figsize=( 10, 4 * len( orig_idx ) ) )
    for row, i in enumerate( orig_idx ):
        ...
        axes[ row ][ 2 ].set_title( "U-Net (pred)", fontsize=8 )
        axes[ row ][ 2 ].axis( "off" )
...
def segment_image_unet( model, img_bgr, threshold=0.5 ):
    #Segmenta una imagen con la U-Net. Retorna la imagen BGR con fondo negro.
    ...
    return cv2.bitwise_and( img_bgr, img_bgr, mask=mask_orig )


def apply_unet_to_dataset( model ):
    #Aplica la U-Net a todas las imágenes y guarda en SEGMENTED_DATASET.
    classes = sorted( [
...
def generate_segmentation_samples( model ):
    #Figura de muestra con resultados de segmentación por clase.
    import random
    random.seed( SEED )
    ...
    for i, class_name in enumerate( classes ):
        ...
        axes[ i ][ 0 ].imshow( img_rgb )
        axes[ i ][ 0 ].set_ylabel( short, fontsize=7, rotation=0, labelpad=75, va="center" )
...
def main( ):
    ...
    print( f"\n  Actualiza DATASET_ROOT en tomato.py para apuntar a '{SEGMENTED_DATASET}'." )
