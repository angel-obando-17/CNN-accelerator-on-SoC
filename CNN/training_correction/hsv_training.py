...
se capa a MAX_CH=64, igual que en training_correction/tomatoV2.py — ver
memoria project_channel_limit_violation.md. MobileNetV1 no usa expansión,
no requiere cambios.
=============================================================================
"""
...
OUTPUT_DIR   = "/mnt/c/Users/ANGEL OBANDO/Documents/Trabajo de grado/CNN/training_correction/resultados_hsv"
...
def segment_hsv( img_bgr ):
    ...
    mask_green = cv2.inRange( hsv, np.array( [ 15,  30,  20 ], dtype=np.uint8 ), np.array( [ 95, 255, 255 ], dtype=np.uint8 ) )

    # Marrón/naranja (manchas de enfermedades, necrosis)
    mask_brown = cv2.inRange( hsv,np.array( [ 5,  50,  20 ], dtype=np.uint8 ), np.array( [ 20, 255, 220 ], dtype=np.uint8 ) )
...
def load_and_preprocess( path, label, target_size ):
    #Carga imagen, aplica HSV threshold, resize y normaliza.
    def _load( p, sz ):
...
def build_mobilenetv1( input_size, num_classes ):
    ...
    cfg = [ ( 64,1 ),( 64,2 ),( 64,1 ),( 64,2 ),( 64,1 ),( 64,2 ), ( 64,1 ),( 64,1 ),( 64,1 ),( 64,1 ),( 64,1 ),( 64,2 ),( 64,1 ) ]
...
def train_model( model, ds_train, ds_val, label ):
    ...
    cbs = [ EarlyStopping( monitor="val_accuracy", patience=7, restore_best_weights=True ), ReduceLROnPlateau( monitor="val_loss", factor=0.5, patience=3, min_lr=1e-6 ) ]
...
def quantize_and_evaluate( model, paths_test, labels_test, class_names, label, target_size, qmode, paths_calib ):
    results = { }
...
# LOOP POR MODELO
def run_model( model_name, builder, paths_train, y_train, paths_val, y_val, paths_test, y_test, class_names, num_classes, summary_rows ):
    print( f"\n{ '='*65 }" )
    ...
    for resolution in RESOLUTIONS:
        ...
        print( f"\n  >> Float32" )
        f32 = evaluate_model( model, ds_test, paths_test, y_test, class_names, label, resolution )
        summary_rows.append( {
        ...
        for qmode in QUANT_MODES:
            print( f"\n  >> { qmode.upper( ) }" )
            try:
                qres = quantize_and_evaluate( model, paths_test, y_test, class_names, label, resolution, qmode, paths_train )
                summary_rows.append( {
                    "modelo": model_name, "segmentacion": "HSV",
...
def save_segmentation_samples( dataset_root, class_names ):
    ...
    fig.suptitle( "Segmentación HSV Threshold — Una muestra por clase", fontsize=13, fontweight="bold", y=1.01 )
...
def main( ):
    ...
    run_model( "MobileNetV1", build_mobilenetv1, paths_train, y_train, paths_val, y_val, paths_test, y_test, class_names, num_classes, summary_rows )

    run_model( "MobileNetV2", build_mobilenetv2, paths_train, y_train, paths_val, y_val, paths_test, y_test, class_names, num_classes, summary_rows )
