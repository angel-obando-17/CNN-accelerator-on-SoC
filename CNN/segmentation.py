...
=============================================================================
Script INDEPENDIENTE — corre esto DESPUÉS de que termine el entrenamiento.
...
def get_tomato_folders(root):
    ...
    for name in os.listdir(root):
        if name.lower().startswith("tomato") and \
...
def make_comparison_grid(folder_path, class_name, n_samples=SAMPLES_PER_CLASS):
    """
    Genera una figura con n_samples filas y 3 columnas:
      Original | Segmentada | Máscara
    """
    ...
    fig.suptitle(f"Segmentación GrabCut — {short_name}", fontsize=13,
...
def generate_mosaic_overview(folders):
    """
    Genera un mosaico general: una imagen por clase (original vs segmentada),
    útil para incluir en el documento del trabajo de grado.
    """
    n_classes = len(folders)
    fig, axes = plt.subplots(n_classes, 2,
                              figsize=(7, 3 * n_classes))
...
if __name__ == "__main__":
    ...
    # 1. Mosaico general (una muestra por clase)
    print("\n  Generando mosaico general...")
    generate_mosaic_overview(folders)

    # 2. Cuadrícula comparativa por clase (N muestras)
    ...
    # 3. Imágenes individuales (opcional — puede tardar varios minutos)
    if SAVE_INDIVIDUALS:
