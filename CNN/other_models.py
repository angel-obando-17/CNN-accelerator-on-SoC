...
def load_and_preprocess( path, label, target_size ):
    #Imágenes ya segmentadas desde el dataset.
    def _load( p, sz ):
...
def mbconv_block( x, filters, strides=1, expand_ratio=2, se_ratio=4, name_prefix="mb" ):
    in_ch  = x.shape[ -1 ]
    exp_ch = in_ch * expand_ratio
    ...
    x_pw = layers.Conv2D( filters, 1, padding="same", use_bias=False, name=f"{name_prefix}_pw" )( x_se )
    x_pw = layers.BatchNormalization( name=f"{name_prefix}_pw_bn" )( x_pw )
...
def build_efficientnet( input_size, num_classes, max_ch=64 ):
    inp = layers.Input( shape=( input_size, input_size, 3 ) )
    x   = layers.Conv2D( min( 32, max_ch ), 3, strides=2, padding="same", use_bias=False, name="stem_conv" )( inp )
    ...
    for i, ( t, c, s ) in enumerate( cfg ):
        x = mbconv_block( x, min( c, max_ch ), strides=s, expand_ratio=t, name_prefix=f"mb{i+1}" )
...
# EVALUACIÓN
def evaluate_model( model, paths_test, labels_test, class_names, model_name, resolution ):
    ds_test = build_dataset( paths_test, labels_test, resolution, BATCH_SIZE, shuffle=False )
    ...
    return { "accuracy": acc, "inference_single_ms": ms_per_img, "inference_batch_s": batch_time }
...
def quantize_and_evaluate( model, paths_test, labels_test, class_names, model_name, target_size, qmode, paths_calib ):
    ...
    return { "accuracy": acc, "inference_single_ms": ms_per_img, "model_size_kb": model_kb, "conversion_s": conversion_time }
...
def main( ):
    ...
    for model_name, builder in model_builders.items( ):
        ...
        print( f"\n  >> Evaluación Float32" )
        f32 = evaluate_model( model, paths_test, y_test, class_names, model_name, RESOLUTION )
        summary_rows.append( {
            "model": model_name,
        ...
        for qmode in QUANT_MODES:
            ...
            try:
                qres = quantize_and_evaluate( model, paths_test, y_test, class_names, model_name, RESOLUTION, qmode, paths_train )
                summary_rows.append( {
                    "model": model_name,
                    "params": model.count_params( ),
                    "quant": qmode,
