...
# =============================================================================
# PIPELINE tf.data (no carga todo en RAM)
# =============================================================================
...
def dw_block(x, filters, strides=1, name_prefix="dw"):
    x = layers.DepthwiseConv2D(3, strides=strides, padding="same",
                                use_bias=False, name=f"{name_prefix}_dw")(x)
    ...
    x = layers.Conv2D(filters, 1, padding="same",
                      use_bias=False, name=f"{name_prefix}_pw")(x)
...
def quantize_and_evaluate(model, paths_test, labels_test, class_names,
    ...
    print(f"  Accuracy {qmode}: {results['accuracy']:.4f}"
          f"  |  {results['inference_single_ms']:.2f} ms/img")
    plot_cm(confusion_matrix(labels_test[idx], y_pred),
            class_names, f"{res_label}_{qmode}")
    return results
    ...
    for resolution in RESOLUTIONS:
        ...
        with Timer("Evaluación") as t_ev:
            eval_res = evaluate_model(
                model, ds_test, paths_test, y_test,
                class_names, f"{res_label}_float32", resolution
            )
