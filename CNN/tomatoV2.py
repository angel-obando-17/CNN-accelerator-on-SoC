...
OUTPUT_DIR   = "/mnt/c/Users/ANGEL OBANDO/Documents/Trabajo de grado/CNN/resultados_mobilenetv2_experimento1"
...
MAX_CH      = 64   # Restricción del acelerador
# =============================================================================
...
def split_paths(file_paths, labels):
    """
    Mismo SEED y mismos parámetros que tomato.py
    → mismo test set → comparación justa con MobileNetV1
    """
...
def load_and_preprocess(path, label, target_size):
    """
    Las imágenes ya vienen segmentadas por U-Net desde el dataset,
    igual que en tomato.py (segment_leaf está comentado allá también).
    """
    def _load(p, sz):
...
# Canales <= 64, mismas operaciones que V1 + residual connections + ReLU6
# =============================================================================

def inverted_residual_block(x, filters, strides=1, expand_ratio=2,
                             name_prefix="irb"):
    in_ch  = x.shape[-1]
    # Sin cap en exp_ch — igual que model_comparison.py que dio 94%
    ...
    # --- Pointwise Conv 1x1 (projection, sin activación) ---
    ...
    # --- Residual connection (solo si stride=1 y canales coinciden) ---
...
def build_mobilenetv2(input_size, num_classes):
    """
    MobileNetV2 restringido — arquitectura idéntica a model_comparison.py
    que dio 94% de accuracy a 256x256.

    Cambios respecto a la versión anterior:
    - 9 bloques (igual que model_comparison.py, no 13)
    - Sin cap en exp_ch dentro del bloque (permite expansión real)
    - Conv 1x1 final antes del GAP (la que marcó la diferencia)
    - Canales de salida por bloque limitados a MAX_CH=64 (restricción acelerador)

    Operaciones usadas — todas soportadas por el acelerador:
    - Conv 3x3 (conv inicial)
    - DW 3x3 + PW 1x1 (dentro de cada bloque)
    - PW 1x1 expansión (Conv 1x1, mismo motor que PW)
    - Add elemento a elemento (residual)
    - ReLU6 (ReLU con clamp, trivial en hardware)
    - GAP
    """
    ...
    # 9 bloques invertidos — misma config que dio 94%
    # (expand_ratio, out_channels, stride)
    cfg = [
        (1, 16, 1),
        (2, 24, 2),
        (2, 24, 1),   # residual
    ...
    for i, (t, c, s) in enumerate(cfg):
        ...
        x = inverted_residual_block(
            x, c_cap, strides=s, expand_ratio=t,
            name_prefix=f"irb{i+1}"
        )

    # Conv 1x1 final — la que marcó la diferencia vs la versión anterior
    # Es una PW, soportada por el acelerador
    ...
    return models.Model(inp, x,
                        name=f"MobileNetV2_restricted_{input_size}")
...
def evaluate_model(model, ds_test, paths_test, labels_test,
                   class_names, res_label, resolution):
    results = {}

    # Inferencia en batch (tf.data, sin explotar VRAM)
    ...
    # Tiempo por imagen individual (sobre 100 muestras)
    times = []
...
def quantize_and_evaluate(model, paths_test, labels_test, class_names,
                           res_label, target_size, qmode, paths_calib):
    results = {}
    ...
    model_path = os.path.join(OUTPUT_DIR,
                              f"model_MobileNetV2_{res_label}_{qmode}.tflite")
    ...
    # Evaluar sobre test set completo
...
def main():
    ...
    # Split — mismo SEED que tomato.py
    (paths_train, y_train,
     paths_val,   y_val,
    ...
    for resolution in RESOLUTIONS:
        ...
        ds_val   = build_dataset(paths_val,   y_val,
                                  resolution, BATCH_SIZE)
        ds_test  = build_dataset(paths_test,  y_test,
                                  resolution, BATCH_SIZE)
        ...
        # Guardar modelo
        keras_path = os.path.join(OUTPUT_DIR,
                                   f"model_MobileNetV2_{res_label}.keras")
        ...
        for qmode in QUANT_MODES:
            ...
            try:
                qres = quantize_and_evaluate(
                    ...
                    res_label, resolution, qmode, paths_train
                )
                summary_rows.append({
                    "resolucion":            res_label,
                    "tiempo_entrenamiento_s": train_time,
    ...
    print(f"\n  [ OK ] Listo manito! Resultados en: {OUTPUT_DIR}/")
